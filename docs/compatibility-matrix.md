...
| VS Code tested | `1.135.0` and `1.136.0`, macOS arm64 | 1.135: real ordinary same-user-data windows and authenticated production AHP turn. 1.136: extension suite, ordinary-window registration, exact-target selection, and AHP 1.0 handshake; the manual task run was blocked when the target window closed normally before preparation | 1.135 Pass for scoped Preview; 1.136 retained-Done UI rerun required |
...
| AHP package | TypeScript package `0.9.0` from pinned upstream commit `f19dd8b3942d029744a3bdd31d830f9428e8ea47`; negotiates AHP `1.0.0` | VS Code 1.135 completed a real turn; VS Code 1.136 advertised `1.0.0` and accepted the same exact offer | Pass; no AHP upgrade is indicated |
...
| Editor Agent Host source (0.4.0 P6) | schema-v2 editor endpoint, Unix socket WebSocket, AHP offer `1.0.0`; standalone fallback | Offline strict parser/socket/selector/lifecycle coverage plus a live authenticated VS Code 1.135.0 editor completion through the owned loopback bridge. The Host subscription objectively echoed the created Session channel | Live execution Pass on tested profile; Chat Sessions UI visibility remains Unverified |
...
| Run `8bbd94cb-0a8f-4471-8e5f-8500dcea0afa` | VS Code 1.136 advertised AHP `1.0.0`; both nodes initially registered and the exact target was selected. The target Extension Host then received a renderer termination request and exited cleanly before Tool preparation. This is evidence of a closed target window, not an AHP incompatibility or product crash. The retained-Done UI outcome was not exercised. |
