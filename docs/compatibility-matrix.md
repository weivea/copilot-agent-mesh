...
| VS Code tested | `1.135.0` and `1.136.0`, macOS arm64 | 1.135 real ordinary same-user-data windows and authenticated production AHP turn; 1.136 accepted the same AHP 1.0 offer, while run `8bbd…` stopped because the target renderer closed before Tool preparation | 1.135 Pass for scoped Preview; 1.136 protocol-compatible, retained-Done rerun still Unverified |
...
| AHP package | TypeScript package `0.9.0` from pinned upstream commit `f19dd8b3942d029744a3bdd31d830f9428e8ea47`; negotiates AHP `1.0.0` | VS Code 1.135.0 handshake and real turn passed; VS Code 1.136.0 accepted the same AHP 1.0 offer | Pass; upstream revision is not yet tagged or npm-published |
