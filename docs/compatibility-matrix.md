...
| VS Code tested | `1.135.0` and `1.136.1`, macOS arm64 | 1.135.0: real ordinary same-user-data windows and authenticated production AHP turn. 1.136.1: controlled read-only dedicated-profile editor discovery, Unix-socket WebSocket upgrade, and exact AHP 1.0.0 initialize | 1.135.0 Pass for scoped Preview; 1.136.1 editor compatibility Pass |
...
| AHP package | TypeScript package `0.9.0` from pinned upstream commit `f19dd8b3942d029744a3bdd31d830f9428e8ea47`; offers exact AHP `1.0.0` | VS Code 1.135.0 real turn passed; VS Code 1.136.1 read-only initialize selected exact runtime protocol `1.0.0` despite registry metadata `0.9.0` | Pass; upstream revision is not yet tagged or npm-published |
...
| Editor Agent Host source (0.4.0 P6) | schema-v2 editor endpoint, known registry metadata `1.0.0` or `0.9.0`, Unix socket WebSocket, exact runtime AHP offer/selection `1.0.0`; standalone fallback | Offline strict parser/socket/selector/lifecycle coverage plus a live authenticated VS Code 1.135.0 editor completion. VS Code 1.136.1 read-only evidence proved registry `0.9.0`, successful socket upgrade, and runtime selection `1.0.0` | Live execution Pass on 1.135.0; read-only compatibility Pass on 1.136.1; Chat Sessions UI visibility remains Unverified |
...
| --- | --- | --- | --- |
| macOS | arm64 | Authenticated same-user-data production AHP start/output/cancel on VS Code 1.135.0; read-only editor discovery/socket/AHP initialize on VS Code 1.136.1; existing exact-build Tunnel evidence | Worker candidate and active Window Node; G0 Go for scoped Preview |
