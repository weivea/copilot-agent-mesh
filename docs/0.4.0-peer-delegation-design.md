...
`renameWindow` 入站消息只携带 action，不携带 `workspaceIdentity`、路径或名称。名称由
Extension Host 的 InputBox 收集；`ProductionDashboardBindings` 在执行瞬间从当前 Window
Node 的 claim 与 active editor 所属 Workspace 派生目标。单一 claimed Workspace 可直接
选择；多根窗口只有 active editor 能唯一绑定到自身 claim 时才允许，否则显式返回安全的
歧义错误。Preview 关闭时面板仍可渲染，但重命名按钮禁用，直接消息调用也显式失败。
