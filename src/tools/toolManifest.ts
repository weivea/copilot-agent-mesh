...
export const MESH_TOOL_MANIFEST_DESCRIPTORS: readonly ToolManifestDescriptor[] = [
	...
	},
	{
		...
		canBeReferencedInPrompt: true,
		modelDescription: 'Delegates a coding task to a selected Copilot Agent Mesh peer and opaque workspace. Omit delegationRequestId for a fresh user invocation; the tool generates and returns one. Reuse that ID only to retry the exact same payload, which recovers the same task; the same ID with a different payload conflicts. A fresh invocation is never deduplicated against completed history. Returns after worker acceptance with a pending task ID; use mesh_get_task to poll. Under a small token budget, compact JSON uses s state (0 accepted pending, 1 reconcile the same intent, 2 error, 3 persistence pending), t taskId, d delegationRequestId, e stable error code, and r retry/reconciliation flag; s=1 or s=3 with r=1 means retry the exact same intent and ID. Do not use this tool for Git, branch, worktree, commit, push, or pull request management.',
