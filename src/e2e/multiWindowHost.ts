...
export async function runWithApi(api: AgentMeshExtensionApi): Promise<void> {
	...
	if (
		...
		throw new Error('The multi-window Extension Host requires exactly one local file workspace.');
	}
			...
			try {
				...
				if (request.action === 'controller.state') {
					...
					if (typeof workspacePath !== 'string' || !isAbsolute(workspacePath)) {
						throw new TypeError('The multi-window workspace path must be absolute.');
					}
